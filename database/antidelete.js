...
module.exports = async(kurr, msg) => {
    try {
        msg = serialize(kurr, msg.message)
...
${type === 'conversation' ? '*Text :* '+chats : ''}`
        if (type === 'conversation'){
            kurr.reply(from, tmxt, msg)
        } else if (type === 'stickerMessage' || type === 'imageMessage' || type === 'videoMessage' || type === 'audioMessage' || type === 'documentMessage'){
            let media = await kurr.downloadMediaMessage(msg)
            kurr.reply(from, tmxt, msg)
            kurr.sendMessage(from, media, type, options)
        } else {
            kurr.reply(from, tmxt, msg)
            kurr.sendMessage(from, msgOpt, type, options)
        }
    } catch (err) {
