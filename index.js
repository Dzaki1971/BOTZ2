...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('Dhani Ofc', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'Dhani Botz Asisstan', 'ubuntu', '3.0' ]
	...
	kurr.on('credentials-updated', () => {
		...
		info('2', 'Dhani Botz Asisstan Loading...')
	})
	...
	kurr.on('open', () => {
		success('2', 'Tersambung Jangan lupa Subs Dhani Ofc')
	})
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6281276698054@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `Dhani Botz Assistan`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `SAYONARA !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6295363217171@s.whatsapp.net","6295363217171@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
